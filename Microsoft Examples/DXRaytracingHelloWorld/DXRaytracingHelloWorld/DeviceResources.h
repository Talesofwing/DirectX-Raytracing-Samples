...
namespace DX {
	...
	public:
		...
		void InitializeDXGIAdapater ();
		void SetAdapterOverride (UINT adapterID) { m_AdapterIDoverride = adapterID; }
		void CreateDeviceResources ();
		void CreateWindowSizeDependentResources ();
		void SetWindow (HWND window, int width, int height);
		bool WindowSizeChagned (int width, int height, bool minimized);
		void HandleDeviceLost ();
		void RegisterDeviceNotify (IDeviceNotify* deviceNotify) {
			m_DeviceNotify = deviceNotify;

			// On RS4 and higher, application that handle device removal
	...
	private:
		...
		// The IDeviceNotify can be held directly as it owns the DeviceResource.
